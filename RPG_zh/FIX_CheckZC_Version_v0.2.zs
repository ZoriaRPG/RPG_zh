float Version[3]={250,0,0}; //Assume 2.50.0 first.
...
void Detect250_2(){
	if ( Link->Action=LA_SCROLLING && Version[ZC_VERSION] < 2.50.2 ) Version[ZC_VERSION] = 250.2;
}
...
void InvertedCircle(int bitmapID, int layer, int x, int y, int radius, int scale, int fillcolor){
	...
	if ( Version[ZC_VERSION] < 2.50.2 ) Screen->DrawBitmap(layer, bitmapID, 0, 0, 256, 176, 0, 56, 256, 176, 0, true); //Draw the bitmap for 2.50.0/1
