const int SEGMENTS_EXPLODE = 1; //Set to '1' if enemy segments should explode, or '0' if they should not. 
...
ffc script BossDeath{
	void run(int enemID, int numExplosions, int victoryMusic, int freezeLink){
		...
		while(true){
			this->X = x;
			this->Y = y;
			if ( NumNPCsOf(enemID) ) { //Check for instances of the enemy to be treated as a boss.
				for ( int q = 1; q <= Screen->NumNPCs(); q++ ) {
					npc n = Screen->LoadNPC(q);
					if ( n->ID == enemID && n->HP > 0 ) { //If it's here, and still alive...
						x = n->X; //Update the FFC coordinates to the position of the boss, or one of its segments. 
						y = n->Y;
						break; //We found one, if there are others, then we'll ignore them, so that we only have explosions when it is finally dead.
					}
					
					//If we've set SEGMENTS_EXPLODE, then we want to check if a segment has died. if so, give it a smaller sxplosion. 
					if ( n->ID == enemID && n->HP <= 0 && SEGMENTS_EXPLODE ) {
						while ( explosionCount > 0 ) {
							...
							for ( int q = 0; q <= numExplosions; q++ ) {
								...
								explosion->X = enemX + Rand(-EXPLOSION_DIST_MINIBOSS,EXPLOSION_DIST_MINIBOSS); //at X coordinates fX plus a slightly randomised offset.
								explosion->Y = enemY + Rand(-EXPLOSION_DIST_MINIBOSS,EXPLOSION_DIST_MINIBOSS); //at Y coordinates fX plus a slightly randomised offset.
								
								explosion2->X = enemX + EXPLOSION_TWO_DIST + Rand(-EXPLOSION_DIST_MINIBOSS,EXPLOSION_DIST_MINIBOSS); //at X coordinates fX plus a slightly randomised offset.
								explosion2->Y = enemY + EXPLOSION_TWO_DIST + Rand(-EXPLOSION_DIST_MINIBOSS,EXPLOSION_DIST_MINIBOSS); //at Y coordinates fX plus a slightly randomised offset.
								
								explosion3->X = enemX + EXPLOSION_THREE_DIST + Rand(-EXPLOSION_DIST_MINIBOSS,EXPLOSION_DIST_MINIBOSS); //at X coordinates fX plus a slightly randomised offset.
								explosion3->Y = enemY + EXPLOSION_THREE_DIST + Rand(-EXPLOSION_DIST_MINIBOSS,EXPLOSION_DIST_MINIBOSS); //at Y coordinates fX plus a slightly randomised offset.
								
								explosion4->X = enemX + EXPLOSION_FOUR_DIST + Rand(-EXPLOSION_DIST_MINIBOSS,EXPLOSION_DIST_MINIBOSS); //at X coordinates fX plus a slightly randomised offset.
								explosion4->Y = enemY + EXPLOSION_FOUR_DIST + Rand(-EXPLOSION_DIST_MINIBOSS,EXPLOSION_DIST_MINIBOSS); //at Y coordinates fX plus a slightly randomised offset.
						...
						if ( explosionCount != numExplosions ) explosioCount = numExplosions;
					}
				}
			}
			
			if ( !NumNPCsOf(enemID) ) { //If there are no enemies on the screen with that ID, then the boss is dead.
				//Play Victory Music Now...
				if ( victoryMusic > 0 ) Game->PlayMIDI(victoryMusic); //If arg for victoryMusic is positive, play a MIDI with that ID.
				if ( victoryMusic < 0 ) Game->PlaySound(victoryMusic); //If it's a negative value, play a WAV from the Game SFX Library with that ID.
				
				
				while ( explosionCount > 0 ) {
					for ( int q = 0; q <= numExplosions; q++ ) {
						if ( freezeLink && !Screen->NumNPCs() ) NoAction(); //Halt Link's Inputs...
						//Run a for loop, to make a timed series of explosions...
						explosion = Screen->CreateEWeapon(EW_SCRIPT1); //Make an explosion..
						explosion2 = Screen->CreateEWeapon(EW_SCRIPT1); //Make an explosion..
						explosion3 = Screen->CreateEWeapon(EW_SCRIPT1); //Make an explosion..
