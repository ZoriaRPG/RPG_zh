...
int SetLastOverworldDMap(int store_dmap, int store_screen){
...
void CheckDMap(){
	...
	if ( PreviousDMap == -1 ) PreviousDMap = Game->GetCurDMAP();
	//if LastOverworldDMap is uninitialised (start of the game)
	if ( LastOverworldDMap == -1 ) LastOverworldDMap = Game->GetCurDMAP();
	...
	if ( DMapChanged && !TimeWarped ) {
		...
		//THis will only be useful at the start of the game. 
		...
		if ( !IsWarpAllowed(Game->GetCurDMap(), WarpAllowedDMaps) && IsWarpAllowed(PreviousMap, WarpAllowedDMaps) ){
		...
		if ( IsDungeonDMap(Game->GetCurDMap(), DungeonDMaps) && IsWarpAllowed(PreviousMap, WarpAllowedDMaps) ) {
			LastOverworldMap = PreviousDMap;
			PreviousDMap = Game->GetCurDMap();
