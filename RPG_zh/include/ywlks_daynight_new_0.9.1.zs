...
void TS_CheckandWarp(int linkdir, bool useffc){
	int RunAfterWarp[]="Ran an instruction after warping Link.";
	int mapset;
	...
	if ( !useffc ) {
		...
		TraceNL(); TraceS(RunAfterWarp); TraceNL();
...
void AreRegistersInitialised(){
	if ( !Game->GetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, UPDATEDFILE_REG) ){
		Game->SetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, LAST_OVERWORLD_REG, -1);
		Game->SetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, LAST_DMAP_REG, -1);
		Game->SetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, UPDATEDFILE_REG, 1);
	}
}
...
int IsTimeWarping(){
	return Game->GetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, TIMEWARPING_REG);
}

void IsTimeWarping(int state){
	Game->SetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, TIMEWARPING_REG, state);
}



void TimeWarped(int state){
	Game->SetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, TIMEWARPED_REG, state);
}

int TimeWarped(){
	return Game->GetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, TIMEWARPED_REG);
}

int GetLastOverworldDMap(){
	return Game->GetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, LAST_OVERWORLD_REG);
}

int GetPreviousDMap(){
	return Game->GetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, LAST_DMAP_REG);
}

void SetPreviousDMap(int value){
	Game->SetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, LAST_DMAP_REG, value);
}

void SetLastOverworldDMap(int value){
	Game->SetDMapScreenD(TIME_OF_DAY_REG_DMAP, TIME_OF_DAY_REG_SCREEN, LAST_OVERWORLD_REG, value);
}
...
void CheckDMap(){
	...
	if ( DMapChanged() && !TimeWarped() ) {
		...
		if ( !IsWarpAllowed(Game->GetCurDMap()) && IsWarpAllowed(GetPreviousDMap()) ){
			SetLastOverworldDMap(GetPreviousDMap());
			SetPreviousDMap(Game->GetCurDMap());
		}
