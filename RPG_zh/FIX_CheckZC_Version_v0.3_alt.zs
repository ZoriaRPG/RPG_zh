...
const int DETECT_250_2_PHASE1 = 3;
const int DETECT_250_2_PHASE2 = 4;
	...
	if ( Version[DETECT_250_1_PASS1] && !Version[DETECT_250_1_PASS2] ) { //Start pass 2.
		for ( int q = 1; q <= Screen->NumEWeapons(); q++ ) { //Look for that eweapon.
			...
			if ( e->Misc[DETECT_250_1_WEAPON_INDEX] == DETECT_250_1_WEAPON_ARB_VAL ) { //by finding its index with our arbitrary value...
				detected250 = true; //If it stille xists after that one Waitframe, then it is 2.50.2, as it would have been removed. 
			}
		}
...
void Detect250_2_Phase1(){
	if ( Link->Action == LA_SCROLLING && !Version[DETECT_250_2_PHASE1] ) {
		//If scrolling is valid before waitdraw, we need to make it invalid before waitframe. 
		Link->Action == LA_NONE;
		Version[DETECT_250_2_PHASE1] = 1;
		Waitframe();
	}
}
...
void Detect250_2_Phase2(){
	if ( Link->Action==LA_SCROLLING && Version[DETECT_250_2_PHASE1] && !Version[DETECT_250_2_PHASE2] && Version[ZC_VERSION] < 250.2 ) Version[ZC_VERSION] = 250.2;
}
