...
const int SLOW_WALK_TIMER_VALUE = 60;
...
global script example{
	void run(){
		...
		ClearWalkTimer();
		//WalkSpeed();
		RunSlowWalkTimer(SLOW_WALK_TIMER_VALUE);
		SlowLink();
		Waitdraw();
...
void RunSlowWalkTimer(int amount){
	if ( IronBoots[SLOW_WALK_TIMER] == ) IronBoots[SLOW_WALK_TIMER] = amount;
	else IronBoots[SLOW_WALK_TIMER]--;
}

void SlowLink(){
	if ( IronBoots() && IronBoots[SLOW_WALK_TIMER] % 2 ) Waitframe();
}
