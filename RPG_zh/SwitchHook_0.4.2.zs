					...
					if ( l->ID == LW_HOOKSHOT ){			//if a hookshot lweapon is present...
						...
						if ( SWITCHHOOK_USE_LAYER_0 ) {
							for (q = 0; q < 176; q++ ) { 	//Pass through the screen combos to find a match...
								...
								if ( ( comboF && SwitchHookVars[SWH_CMB] == comboF ) || ( !comboF && SwitchHookVars[SWH_CMB] == CF_SWITCHHOOK ) ) { 	
						...
						if ( SWITCHHOOK_USE_LAYER_1 ) {
							for (q = 0; q < 176; q++ ) { 	//Pass through the screen combos to find a match...
								...
								if ( ( comboF && SwitchHookVars[SWH_CMB1] == comboF ) || ( !comboF && SwitchHookVars[SWH_CMB1] == CF_SWITCHHOOK ) ) { 	
												//If D0 is assigned a value above 0, check to see if it matches
												//that combo inherent flag, otherwise check if it matches the one
												//assigned to the constant CF_SWITCHSHOT.
						...
						if ( SWITCHHOOK_USE_LAYER_2 ) { //2
							for (q = 0; q < 176; q++ ) { 	//Pass through the screen combos to find a match...
								...
								if ( ( comboF && SwitchHookVars[SWH_CMB2] == comboF ) || ( !comboF && SwitchHookVars[SWH_CMB2] == CF_SWITCHHOOK ) ) { 	
												//If D0 is assigned a value above 0, check to see if it matches
												//that combo inherent flag, otherwise check if it matches the one
												//assigned to the constant CF_SWITCHSHOT.
									...
									//if ( switching ) {
									if ( __SwitchHookComboCollision(SwitchHookVars[SWH_LOC],l) ) {	//and there is collision between that combo and the hookshot...
