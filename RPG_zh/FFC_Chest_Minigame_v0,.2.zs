...
ffc script ChestMiniGame{	//layer for combos, number of plays, big prize, hold up animation, Screen->D register, 
	void run(int chestLayer, int max_chests_Link_can_open, int specialPrize, int holdUpType, int reg, int percentMainprize, int backupPrize, int costPerPlay){
		...
		Screen->Message(MSG_CHEST_GAME_RULES);
		
		while(true) {
			
			if ( gameOver ) break;
			if ( LinkCollision(this) && Game->Counter[CR_RUPEES] > costPerPlay && Xor(Link->PressA,Link->PressB) ) {
				gameRunning = true;
			...
			if ( gameRunning ) {
				...
				if ( Link->InputUp && openchest ) { //The timer is over, and Link is still pressing up, so open the chest.
