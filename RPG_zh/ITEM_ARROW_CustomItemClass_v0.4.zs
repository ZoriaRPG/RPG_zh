	...
	void run(int sound, int counter, int errorSFX, int maxOnscreen, int allowLinkZ, int arrowID, int sprite, int dist){
		...
		if ( counter ) {
			if ( Game->Counter[counter] && !maxOnscreen ){
				...
				//arrow->Level = normalArrow->Level;
				//arrow->Counter = normalArrow->Counter;
				//arrow->MaxIncrement = normalArrow->MaxIncrement;
				//arrow->Keep = normalArrow->Keep;
				//arrow->Max = normalArrow->Max;
				//arrow->Amount = normalArrow->Amount;
				//arrow->UseSound = normalArrow->UseSound;
			}
