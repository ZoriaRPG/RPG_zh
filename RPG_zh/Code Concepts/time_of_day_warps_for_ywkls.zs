 ...
 TS_DMaps[13] = {	0, 
			...
			0-3,4-7,32-35,71-74,75-78,79-82,83-86,87-90,91-94,95-98,101-104,105-108,109-112
...
//Returns the current DMap set.
int GetDMapSet(){
	int curmap = Game->GetCurDMap();
	int mapset;
	if ( curmap >=0 && curmap <= 3 ) mapset = MAPSET2;
	if ( curmap == 91 || curmap == 79 || curmap == 83 || curmap == 87 ) mapset = MAPSET2;
	if ( curmap == 95 || curmap == 101 || curmap == 105 || curmap == 109 ) mapset = MAPSET3;
	return mapset;
}
...
void DoWarp(int linkdir){
	...
	int warpMaps[]={	0,1,2,3,
		...
		Link->PitWarp(warpMaps[mapset+linkdir]);
}
