...
item script heartjar{
    void run(int msg, int sfx, imt errsfx){
        if ( Link->HP < Link->MaxHP ) {
            int diff = Link->MaxHP - Link->HP;
            int heart_res = ReserveCounters[RESERVE_HEARTS];
            if ( heart_res > ( Ceiling(diff / 16) ) ) {
                ReserveCounters[RESERVE_HEARTS] -= ( Ceiling(diff / 16) );
                Link->HP += diff;
                Game->PlaySound(sfx);
            }
            if ( heart_res < ( Ceiling(diff / 16) ) && ReserveCounters[RESERVE_HEARTS] ){
...
item script magicjar{
    void run(int msg, int sfx, imt errsfx){
        if ( Link->MP < Link->MaxMP ) {
            int diff = Link->MaxMP - Link->MP;
            int mag_res = ReserveCounters[RESERVE_MAGIC];
            if ( mag_res > ( Ceiling(diff / 32) ) ) {
                ReserveCounters[RESERVE_MAGIC] -= ( Ceiling(diff / 32) );
                Link->MP += diff;
                Game->PlaySound(sfx);
            }
            if ( mag_res < ( Ceiling(diff / 32) ) && ReserveCounters[RESERVE_MAGIC] ){
