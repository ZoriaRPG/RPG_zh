...
//Register of Screen->D to store version number. This will be used on EVERY SCREEN in a game.
...
const int ZC_VERSION_VALIDATE_REGISTER = 7;
...
//Sets the flags for the version number on every screen, of every DMap in a game, 
//so that it is always possible to read them.
//Call as: SetVersionFlags(Screen->D[ZC_VERSION]);
void SetVersionFlags(int val){
	for ( int q = 0; q < TOTAL_DMAPS; q++ ) {
		for ( int w = 0l w < 128; w++ ) {
			Game->SetDMapScreenD(q, w, ZC_VERSION, val); 	
}
